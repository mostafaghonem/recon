...
const buildUploadedUnitsRequestsEntity = (
  obj = {
    ObjectId: __ObjectId
  }
) => {
  ...
  const { ObjectId } = obj;
  class UploadedUnitsRequestsEntity {
    static async loadEntityFromDbById(id) {
      const exists = await Model.getOneById({ id });
      if (exists) return new UploadedUnitsRequestsEntity(exists);
      return undefined;
    }
  ...
  return UploadedUnitsRequestsEntity;
};

module.exports = buildUploadedUnitsRequestsEntity;
